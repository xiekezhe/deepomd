...
ABSL_FLAG(double, cfr_rm_ub, 1e10, "cfr regret match parameter.");
...
ABSL_FLAG(int, global_value_memory_size, 10000,
          "Memory size for training cfr value.");
...
ABSL_FLAG(int, global_value_batch_size, 256,
          "How many states to learn from per batch.");
ABSL_FLAG(int, inference_batch_size, 1,
          "How many threads to wait for for inference.");
ABSL_FLAG(int, train_steps, 2, "How many steps for training cfr value.");
ABSL_FLAG(int, global_value_train_steps, 2,
          "How many steps for training cfr value.");
ABSL_FLAG(int, policy_train_steps, 2,
...
int main(int argc, char** argv) {
  ...
  config.global_value_memory_size =
      absl::GetFlag(FLAGS_global_value_memory_size);
  ...
  config.global_value_batch_size = absl::GetFlag(FLAGS_global_value_batch_size);
  config.inference_batch_size = absl::GetFlag(FLAGS_inference_batch_size);
  config.train_steps = absl::GetFlag(FLAGS_train_steps);
  config.global_value_train_steps =
      absl::GetFlag(FLAGS_global_value_train_steps);
