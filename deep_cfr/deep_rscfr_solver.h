
// Copyright 2019 DeepMind Technologies Ltd. All rights reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#ifndef THIRD_PARTY_OPEN_SPIEL_ALGORITHMS_ROBUST_SAMPLING_MCCFR_H_
#define THIRD_PARTY_OPEN_SPIEL_ALGORITHMS_ROBUST_SAMPLING_MCCFR_H_

#include <memory>
#include <random>
#include <unordered_set>
#include <vector>

#include "deep_escfr_solver.h"
#include "open_spiel/algorithms/cfr.h"
#include "open_spiel/algorithms/public_tree.h"
#include "open_spiel/policy.h"
#include "open_spiel/spiel.h"

namespace open_spiel {
namespace algorithms {

enum class SamplingType { kUniform, kAverage };

class DeepRSCFRSolver : public DeepESCFRSolver {
 public:
  static constexpr int kSampling = 3;
  static constexpr SamplingType kSamplingType = SamplingType::kUniform;

  DeepRSCFRSolver(const Game& game,
                  std::shared_ptr<VPNetEvaluator> value_0_eval_,
                  std::shared_ptr<VPNetEvaluator> value_1_eval_,
                  std::shared_ptr<VPNetEvaluator> policy_eval,
                  bool use_regret_net, bool use_policy_net, bool use_tabular,
                  std::mt19937* rng,
                  AverageType avg_type = AverageType::kCurrent)
      : DeepESCFRSolver(game, value_0_eval_, value_1_eval_, policy_eval,
                        use_regret_net, use_policy_net, use_tabular, rng,
                        avg_type) {}

  void clear_updated() {
    updated_infos.clear();
    updated_info_tensor.clear();
  }

 protected:
  double UpdateRegrets(PublicNode* node, Player player, double player_reach,
                       double oppoment_reach, double sampling_reach,
                       Trajectory& value_trajectory,
                       Trajectory& policy_trajectory, int step,
                       std::mt19937* rng,
                       const ChanceData& chance_data) override;

  std::unordered_set<std::string> updated_infos;
  std::unordered_set<uint64_t> updated_info_tensor;
};

}  // namespace algorithms
}  // namespace open_spiel
#endif  // THIRD_PARTY_OPEN_SPIEL_ALGORITHMS_ROBUST_SAMPLING_MCCFR_H_