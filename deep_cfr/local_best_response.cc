...
logic::CardSet LocalBestResponse::_deal_cards(int required_cards,
                                              const logic::CardSet &hole_cards,
                                              const logic::CardSet &outcome,
                                              std::mt19937 *rng) {
  std::vector<uint8_t> deck_r = deck_.ToCardArray();
  logic::CardSet new_outcome = outcome;
  while (new_outcome.NumCards() < required_cards) {
    std::uniform_int_distribution<uint8_t> rnd(0, deck_r.size() - 1);
    int card_index;
    while (true) {
      card_index = rnd(*rng);
      if (!hole_cards.ContainsCards(deck_r[card_index]) &&
          !new_outcome.ContainsCards(deck_r[card_index])) {
        break;
      }
    }
    new_outcome.AddCard(deck_r[card_index]);
  }
  return new_outcome;
}
  ...
  if (rest_num_cards == 0) {
    ...
    for (int b = 0; b != rollout_batch_size; ++b) {
      ...
      logic::CardSet new_outcome =
          _deal_cards(num_board_cards_, hole_cards, outcome, rng);
...
double LocalBestResponse::_dist_br_chance(Player player,
  ...
  logic::CardSet new_outcome =
      _deal_cards(required_cards, hole_cards, outcome, rng);
