...
void actor(const Game& game, const DeepCFRConfig& config, int num,
  ...
  SbCFR::Mode mode = SbCFR::GetMode(config.cfr_mode);
  SbCFR solver(game, value_0_eval, value_1_eval, policy_0_eval, policy_1_eval,
               config.use_regret_net, config.use_policy_net, config.use_tabular,
               &rng, cfr_batch_per_actor, config.cfr_rm_scale,
               config.cfr_rm_amp, config.cfr_rm_damp, mode);
  for (int step = 1; !stop->StopRequested() &&
                     (config.max_steps == 0 || step <= config.max_steps);
       ++step) {
    for (Player player = 0; player < 2; ++player) {
      std::vector<bool> recover_vec = std::vector<bool>{true, false};
      {
      ...
      for (bool recover : recover_vec) {
        ...
        if (recover) {
          for (int game_num = 1;
               ...
               ++game_num) {
            auto trajectories =
                solver.RunIteration(player, step, game_num, recover);
      ...
      // logger->Print(
      //     "Step %d\npush %d trajectories to queue %d, queue size: %d -> "
      //     "%d, policy size: %d -> %d.",
      ...
      // NOTE: Do not break here, otherwise some threads may be blocked
      // here. The last thread calls the Block is responsible to set the
      // Learning flag before the other threads existing the block.
...
LearningInfo learn_imp_(const Game& game, const DeepCFRConfig& config,
  ...
  // }
  {
    ...
    for (int i = 0; i < train_steps; i++) {
      ...
      if (value_or_policy) {
        loss +=
            learn_model->TrainValue(player, train_inputs, config.learning_rate);
      } else {
  ...
  std::string checkpoint_path = SaveAndLoad(config, device_manager, step,
                                            value_or_policy, player, device_id);
...
void learner(const Game& game, const DeepCFRConfig& config,
  ...
  for (int step = 1; !stop->StopRequested() &&
    ...
    for (int player = 0; player < 2; ++player) {
      LearningInfo learn_info;
      {
        // NOTE: sbcfr will clear its buffer before training.
        value_replay_buffer[player].Clear();
        learn_info = learn_imp_(
            game, config, device_manager, device_id, logger, rng, step,
            value_trajectory_queue, value_replay_buffer[player], true, player,
        ...
        if (config.inference_cache > 0 && player == 0) {
          cache_info = value_eval[player]->CacheInfo();
        }
        value_eval[player]->ClearCache();
      }
      {
        // NOTE: the samples are for training the opponent's policy net.
        Player opponent = player == 0 ? 1 : 0;
        learn_info = learn_imp_(
            game, config, device_manager, device_id, logger, rng, step,
            policy_trajectory_queue, policy_replay_buffer[opponent], false,
            opponent, config.policy_memory_size, config.cfr_batch_size,
            config.policy_train_steps, config.train_batch_size, stop);
        policy_eval[opponent]->ClearCache();
        sampled_states += learn_info.num_states;
        trained_states += learn_info.trained_states;
        num_states += learn_info.num_states;
        total_trajectories += learn_info.num_trajectories;
      ...
      if (player == 1 &&
        ...
        {
          std::unique_lock<std::mutex> lock(EvaluatingMutex);
          NotEvaluatingCondition.wait(lock, [stop]() {
            return (!Evaluating) || stop->StopRequested();
          });
        }
      }
      {
...
bool deep_cfr(DeepCFRConfig config, StopToken* stop) {
  ...
  for (int i = 0; i < config.num_cpus; ++i) {
    device_manager.AddDevice(CFRNetModel(
        *game, config.path, model_path, config.graph_def, config.omp_threads,
        "/cpu:0", config.use_regret_net, true, config.use_policy_net, true));
  }
  ...
  ThreadedQueue<Trajectory> value_trajectory_queue(config.cfr_batch_size * 2);
  ThreadedQueue<Trajectory> policy_trajectory_queue(config.cfr_batch_size * 2);
  ...
  for (int i = 0; i < config.actors; ++i) {
    actors.emplace_back([&, i]() {
      actor(*game, config, i, &value_trajectory_queue, &policy_trajectory_queue,
            value_0_eval, value_1_eval, policy_0_eval, policy_1_eval,
            &actor_barrier, stop);
    });
  }
  ...
  learner(*game, config, &device_manager, value_0_eval, value_1_eval,
          policy_0_eval, policy_1_eval, &value_trajectory_queue,
