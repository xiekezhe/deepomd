...
labels_set = [["Neural CFR-B v.s. ABS-CFR",
               "Deep CFR v.s. ABS-CFR", "Neural CFR-B v.s. Deep CFR"]]

names_set = [["Neural CFR-B v.s. ABS-CFR",
              "Deep CFR v.s. ABS-CFR", "Neural CFR-B v.s. Deep CFR"]]

data_set = [["match_HULH_abs_ossbcfr_post_20.txt",
             "match_HULH_abs_deep_cfr_post_20.txt", "match_HULH_deep_cfr_ossbcfr_post_20.txt"]]
...
for ind, (labels, names) in enumerate(zip(labels_set, names_set)):
    ...
    for j, (name, label) in enumerate(zip(names, labels)):
        ...
        ax.plot(np.arange(0, len(means) + 1), np.zeros(len(means) + 1), '--k')
        ax.set_xlim([0, len(means) + 1])
        ax.set_xticks(np.arange(0, 21, 5))

    ax.set_xlabel('Days')
    ax.set_ylabel('Expected value (mbb/g)')
    ax.legend()
    fig.tight_layout(pad=0.5, w_pad=0.5, h_pad=0.5)

    fig.savefig('./paper/plot_match_' + str(ind) + '.pgf')
    fig.savefig('./paper/plot_match_' + str(ind) + '.pdf')
