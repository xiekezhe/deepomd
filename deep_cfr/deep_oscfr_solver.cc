...
double DeepOSCFRSolver::UpdateRegrets(
  ...
  if (cur_player == player) {
    ...
    if (!use_regret_net || use_tabular) {
      ...
      value_eval_[cur_player]->AccumulateCFRTabular(state, linear_regret);
    }
    if (use_regret_net) {
      ...
      value_trajectory.states.push_back(ReplayNode{
          is_key, information_tensor, cur_player, legal_actions, regret,
          player_reach, (double)step * opponent_reach / sampling_reach});
    }
  }
  if (avg_type_ == AverageType::kSimple && cur_player != player) {
    if (!use_policy_net || use_tabular) {
      ...
      policy_eval_->AccumulateCFRTabular(state, linear_policy);
    }
    if (use_policy_net) {
      ...
      policy_trajectory.states.push_back(ReplayNode{
          is_key, information_tensor, cur_player, legal_actions, policy,
          opponent_reach, (double)step * opponent_reach / sampling_reach});
    }
  }
  return value;
}
