...
LearningInfo learn_imp_(const Game& game, const DeepCFRConfig& config,
  ...
  int64_t trained_states = 0;
  {
    ...
    for (int train_step = 0; train_step < train_steps; train_step++) {
      ...
      if (value_or_policy_or_global == 0) {
        ...
        target_outputs =
            learn_model->InfCurrentPolicy(player, target_inference_inputs);
        // train current_policy net.
        for (int sample_idx = 0; sample_idx != data.size(); ++sample_idx) {
          const auto& node = data[sample_idx];
          std::vector<double> target_policy(node.legal_actions.size());
          std::vector<double>& current_policy =
              target_outputs[sample_idx].value;
          const std::vector<double>& regret = node.value;
          double policy_norm = 0;
          for (int aidx = 0; aidx != node.legal_actions.size(); ++aidx) {
            policy_norm += current_policy[aidx] * current_policy[aidx];
          }
          policy_norm = std::sqrt(policy_norm);
          for (int aidx = 0; aidx != node.legal_actions.size(); ++aidx) {
            target_policy[aidx] =
                std::max(current_policy[aidx] / policy_norm *
                                 std::sqrt(alpha * node.opponent_reach) +
                             regret[aidx],
                         0.0);
          }
          CFRInfoStateValues info_state_copy(node.legal_actions, 0.0);
          info_state_copy.SetRegret(target_policy);
          info_state_copy.ApplyRegretMatching();
          target_policy = info_state_copy.current_policy;
          train_inputs.emplace_back(CFRNetModel::TrainInputs{
              node.info_str, node.legal_actions, node.legal_actions,
              node.information, target_policy, 1});
        }
  ...
  if (config.sync_by_restore) {
    std::string checkpoint_path =
        SaveAndLoad(config, device_manager, -1, value_or_policy_or_global,
                    player, device_id);
    if (config.verbose) {
  ...
  for (int step = 1; !stop->StopRequested() &&
    ...
    for (int player = 0; player < 2; ++player) {
      ...
      }
      {
        ...
        auto learn_info =
            learn_imp_(game, config, device_manager, device_id, logger, rng,
                       step, policy_trajectory_queue, policy_replay_buffer, 1,
                       player, config.policy_memory_size, config.cfr_batch_size,
                       config.policy_train_steps, config.train_batch_size,
                       alpha, cfr_value, is_training, stop);
        policy_evals[player]->ClearCache();
...
bool deep_cfr(DeepCFRConfig config, StopToken* stop) {
  ...
  if (config.graph_def.empty()) {
    config.graph_def =
        absl::StrJoin({config.game, std::string("ossbcfr")}, "_");
