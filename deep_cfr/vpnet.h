...
class CFRNetModel {
 ...
 public:
  ...
  std::vector<CFRNetModel::InferenceOutputs> InfzPolicy(
      Player player, const std::vector<InferenceInputs>& inputs,
      bool normalize = true, bool use_target = false) {
    std::vector<CFRNetModel::InferenceOutputs> ret;
    if (use_value_net) {
      ret = policy_z_nets[player]->Inference(inputs, use_target);
    } else {
      ret = current_policy_table->Inference(inputs, use_target);
    }
    if (normalize) {
  ...
  double TrainzPolicy(Player player,
                            const std::vector<TrainInputs>& inputs, double lr) {
    if (use_value_net) {
      return policy_z_nets[player]->Learn(inputs, lr, 1.0);
    }
    return 0;
  }
 ...
 private:
  ...
  std::vector<std::shared_ptr<CFRNet>> policy_z_nets;  //new modified
