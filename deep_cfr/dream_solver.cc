...
double DeepOSSBCFRSolver::UpdateRegrets(
  ...
  if (step != 1) {
    ...
    CFRNetModel::InferenceInputs inference_input{is_key, legal_actions,
                                                 information_tensor};
    auto cfr_regret =
        value_eval_[cur_player]->Inference(cur_player, inference_input).value;
    current_info_state.SetRegret(cfr_regret);
    current_info_state.ApplyRegretMatchingUsingEpsilonMax();
  ...
  if (cur_player == player) {
    ...
    sub_value = UpdateRegrets(
        ...
        policy_trajectory, history_trajectory, step, rng, chance_data);
    m[aidx] =
        m[aidx] + (sub_value - m[aidx]) / info_state_copy.current_policy[aidx];
    for (int bidx = 0; bidx != legal_actions.size(); ++bidx) {
      value += m[bidx] * current_info_state.current_policy[bidx];
    }
    for (int bidx = 0; bidx != legal_actions.size(); ++bidx) {
      regret[bidx] = (m[bidx] - value);
    }
  } else {
    ...
    double new_ave_reach = ave_opponent_reach * 1.0 / legal_actions.size();
    double new_sampling_reach =
    ...
    sub_value = UpdateRegrets(
        ...
        history_trajectory, step, rng, chance_data);
    m[aidx] = m[aidx] +
              (sub_value - m[aidx]) / current_info_state.current_policy[aidx];
    for (int bidx = 0; bidx != legal_actions.size(); ++bidx) {
      value += m[bidx] * current_info_state.current_policy[bidx];
    }
  }
  ...
  history_trajectory.states.push_back(
      ReplayNode{obs_key, state_tensor, player, legal_actions,
                 current_info_state.current_policy, m, aidx, 1.0, player_reach,
  ...
  if (cur_player == player) {
    ...
    value_trajectory.states.push_back(
        ReplayNode{is_key, information_tensor, cur_player, legal_actions,
                   current_info_state.current_policy, regret, aidx,
                   double(step), player_reach, opponent_reach, sampling_reach});
  }
