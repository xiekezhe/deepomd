...
VPNetEvaluator::VPNetEvaluator(DeviceManager *device_manager,
                               ...
                               bool current, bool z,bool global, int batch_size,
                               int threads, int cache_size, int cache_shards,
                               int device_id)
    : device_manager_(*device_manager),
      device_id_(device_id),
      value_or_policy_(value_or_policy),
      player_(player),
      current_(current),
      z_(z),
      global_(global),
...
void VPNetEvaluator::SetCFRTabular(const CFRNetModel::TrainInputs &input) {
  for (auto &device : device_manager_.GetAll()) {
    if (value_or_policy_) {
      if (global_) {
        device->SetCFRTabularGlobalValue(input);
      } else {
        device->SetCFRTabularValue(input);
      }
    } else {
      if (current_) {
        device->SetCFRTabularCurrentPolicy(input);
      } else {
...
void VPNetEvaluator::AccumulateCFRTabular(
  ...
  for (auto &device : device_manager_.GetAll()) {
    if (value_or_policy_) {
      if (global_) {
        device->AccumulateCFRTabularGlobalValue(input);
      } else {
        device->AccumulateCFRTabularValue(input);
      }
    } else {
      if (current_) {
        device->AccumulateCFRTabularCurrentPolicy(input);
      } else {
...
std::vector<double> VPNetEvaluator::GetCFRTabular(
  ...
  if (value_or_policy_) {
    if (global_) {
      return device_manager_.Get(1, device_id_)
          ->GetCFRTabularGlobalValue(input);
    } else {
    ...
    if (current_) {
      return device_manager_.Get(1, device_id_)
          ->GetCFRTabularCurrentPolicy(input);
    } else {
