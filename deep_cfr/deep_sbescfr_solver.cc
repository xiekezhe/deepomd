...
DeepSbESCFRSolver::DeepSbESCFRSolver(
    ...
    : game_(game.Clone()),
      ...
      policy_z_eval_(policy_z_eval),//new modified
...
std::vector<Trajectory> DeepSbESCFRSolver::RunIteration(std::mt19937* rng,
  ...
  Trajectory policy_z_trajectory;//new modified
  // Sample a chace seed at the start of an iteration.
  ChanceData chance_data = root_state_->SampleChance(rng);
  double predicted_values = 0;
  double value = UpdateRegrets(root_node_, player, 1, 1, 1, 1, predicted_values,value_trajectory,
                               policy_trajectory,policy_z_trajectory, step, rng, chance_data);
  value_trajectory.node_touched = node_touch_;
  value_trajectory.value = value;
  return {value_trajectory, policy_trajectory,policy_z_trajectory};//new modified
}

double DeepSbESCFRSolver::UpdateRegrets(
    PublicNode* node, Player player, double player_reach, double opponent_reach,
    double ave_opponent_reach, double sampling_reach,double &predicted_values,
    Trajectory& value_trajectory, Trajectory& policy_trajectory, Trajectory& policy_z_trajectory,int step,//new modified
  ...
  if (state.IsTerminal()) {
    predicted_values=state.PlayerReturn(player);  //new modified
    return state.PlayerReturn(player);
  } else if (state.IsChanceNode()) {
    double values = 0.0;
    double pv = 0.0;
    Action action = SampleAction(state.ChanceOutcomes(), dist_(*rng)).first;
    values = UpdateRegrets(node->GetChild(action), player, player_reach,
                         opponent_reach, ave_opponent_reach, sampling_reach,pv,
                         value_trajectory, policy_trajectory,policy_z_trajectory, step, rng,//new modified
                         chance_data);
    predicted_values = pv ;
    return  values ;
...
if (step != 1) {
    ...
    auto cfr_policy = current_policy_eval_[cur_player]
                          ...
                          .value;
    //new modified get z policy
    auto policy_z = policy_z_eval_[cur_player]
                          ->Inference(cur_player, inference_input)
                          .value;
    // get ave policy , new add ,reslts not good 2022.11.14
    ...
    info_state_copy.SetPolicy(policy_z); //new modified
  }
  double value = 0.0;
  std::vector<double> child_values(legal_actions.size(), 0.0);
  std::vector<double> predicted_child_values(legal_actions.size(), 0.0);
  ...
  if (cur_player == player) {
    ...
    for (int aidx = 0; aidx < legal_actions.size(); ++aidx) {
      ...
      child_values[aidx] = UpdateRegrets(
          ...
          opponent_reach, ave_opponent_reach, sampling_reach, pv,value_trajectory,
          policy_trajectory, policy_z_trajectory,step, rng, chance_data);//new modified
      value += current_info_state.current_policy[aidx] * child_values[aidx];
      predicted_child_values[aidx]= pv;
      predicted_values += current_info_state.current_policy[aidx] *pv;
    }
  } else {
    ...
    //double new_ave_reach = current_info_state.cumulative_policy[aidx] * ave_opponent_reach;  reslts not good 2022.11.14
    ...
    value = UpdateRegrets(node->GetChild(legal_actions[aidx]), player,
                          ...
                          new_sampling_reach, pv,value_trajectory,
                          policy_trajectory,policy_z_trajectory, step, rng, chance_data);//new modified
    predicted_values = pv;
  }
  ...
  if (cur_player == player) {
    ...
    CFRInfoStateValues next_z_info_state(legal_actions, kInitialTableValues);
    std::vector<double> next_policy(legal_actions.size());
    std::vector<double> next_z_policy(legal_actions.size());
    std::vector<double> next_z_regret(legal_actions.size());
    ...
    if (step == 0) {
      ...
      value = cfrplus_backward_update_regret(
          ...
          child_values, predicted_sub_values, info_state_copy.current_policy, step);//new modified
      for (int aidx = 0; aidx < legal_actions.size(); ++aidx) {
       next_z_policy[aidx]= next_z_regret[aidx] =std::max(predicted_sub_values[aidx] - value, 0.0);
          
    }
      
    next_z_info_state.SetRegret(next_z_policy);
    next_z_info_state.ApplyRegretMatching();
    next_z_policy = next_z_info_state.current_policy;
    for (int aidx = 0; aidx != legal_actions.size(); ++aidx) {
      SPIEL_CHECK_TRUE(next_z_policy[aidx] >= 0 && next_policy[aidx] <= 1);
    }
    }
    predicted_values = cfrplus_backward_update_regret(
          alpha_,
          delta * delta * legal_actions.size() * Z_weight * ave_opponent_reach,
          predicted_child_values, sub_values, next_z_info_state.current_policy, step);
    ...
    if (use_regret_net) {
      value_trajectory.states.push_back(ReplayNode{
          ...
          delta * delta * legal_actions.size() * Z_weight * ave_opponent_reach,
          sampling_reach});
      policy_z_trajectory.states.push_back(ReplayNode{
          is_key, information_tensor, cur_player, legal_actions, next_z_policy,
          next_z_regret, -1, 1.0, player_reach,
          delta * delta * legal_actions.size() * Z_weight * ave_opponent_reach,
          sampling_reach});
    }
  }
