...
class SbCFR {
 ...
 public:
  ...
  static Mode GetMode(const std::string &mode_str) {
    if (mode_str == "CFR") {
      return Mode::kCFR;
    } else if (mode_str == "OSCFR") {
      return Mode::kOSCFR;
    } else if (mode_str == "SubCFR") {
      return Mode::kSubCFR;
    } else if (mode_str == "PreSbCFR") {
      return Mode::kPreSbCFR;
    } else if (mode_str == "SubPreSbCFR") {
      return Mode::kSubPreSbCFR;
    } else if (mode_str == "PostSbCFR") {
      return Mode::kPostSbCFR;
    } else if (mode_str == "SubPostSbCFR") {
      return Mode::kSubPostSbCFR;
    } else if (mode_str == "SbCFR") {
      return Mode::kSbCFR;
    } else if (mode_str == "SubSbCFR") {
  ...
  std::pair<Trajectory, Trajectory> RunIteration(Player player, int iterations,
                                                 int step);

  std::pair<Trajectory, Trajectory> RunIteration(std::mt19937 *rng,
                                                 Player player, int iterations,
                                                 int step);
 ...
 private:
  ...
  Eigen::ArrayXd _enter_cfr(Player player, algorithms::PublicNode *node,
                            ...
                            double sampler_reach, const logic::CardSet &outcome,
 ...
 private:
  ...
  std::unordered_map<std::string, ReplayNode> info_policies_;
  ...
  int num_threads_;
  std::mutex alpha_mutex_;
  std::mutex node_mutex_;
  std::mutex info_policy_mutex_;
  std::mutex info_regret_mutex_;
