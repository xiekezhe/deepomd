...
void CreateModel(const Game& game, double learning_rate, double weight_decay,
  ...
  std::vector<std::string> model_names{"value_0",   "value_1",  "global_0",
                                       "global_1",  "policy_0", "policy_1","policy_z_0","policy_z_1",   //new modified
                                       "current_0", "current_1"};
  for (auto& model_name : model_names) {
    std::string nn_model = model_name.find("policy") == std::string::npos
                               ? regret_nn_model
                               : policy_nn_model;
    if (model_name.find("global") != std::string::npos) {
      SPIEL_CHECK_TRUE(CreateGraphDef(
          game.ObservationTensorShape(), game.NumDistinctActions(),
          learning_rate, weight_decay, model_path, "/cpu:0",
          absl::StrJoin({model_name_prefix, model_name, std::string("cpu.pb")},
                        "_"),
          nn_model, nn_width, nn_depth));
    } else {
      SPIEL_CHECK_TRUE(CreateGraphDef(
          game.InformationStateTensorShape(), game.NumDistinctActions(),
          learning_rate, weight_decay, model_path, "/cpu:0",
          absl::StrJoin({model_name_prefix, model_name, std::string("cpu.pb")},
                        "_"),
          nn_model, nn_width, nn_depth));
    ...
    if (use_gpu) {
      if (model_name.find("global") != std::string::npos) {
        SPIEL_CHECK_TRUE(CreateGraphDef(
            game.ObservationTensorShape(), game.NumDistinctActions(),
            learning_rate, weight_decay, model_path, "/gpu:0",
            absl::StrJoin(
                {model_name_prefix, model_name, std::string("gpu.pb")}, "_"),
            nn_model, nn_width, nn_depth));
      } else {
        SPIEL_CHECK_TRUE(CreateGraphDef(
            game.InformationStateTensorShape(), game.NumDistinctActions(),
...
CFRNetModel::CFRNetModel(const Game& game, const std::string& path,
  ...
  if (use_value_net) {
    value_nets.emplace_back(new CFRNet(game, path, model_path,
                                       file_name + "_value_0", num_threads_,
                                       device, use_target));
    current_policy_nets.emplace_back(
        new CFRNet(game, path, model_path, file_name + "_current_0",
                   num_threads_, device, use_target));
    global_value_nets.emplace_back(
        new CFRNet(game, path, model_path, file_name + "_global_0",
                   num_threads_, device, use_target));
    policy_z_nets.emplace_back(                                        //new modified
        new CFRNet(game, path, model_path, file_name + "_policy_z_0",
                   num_threads_, device, use_target));
    if (seperate_value_net) {
      value_nets.emplace_back(new CFRNet(game, path, model_path,
                                         file_name + "_value_1", num_threads_,
                                         device, use_target));
      global_value_nets.emplace_back(
          new CFRNet(game, path, model_path, file_name + "_global_1",
                     num_threads_, device, use_target));
      current_policy_nets.emplace_back(
          new CFRNet(game, path, model_path, file_name + "_current_1",
                     num_threads_, device, use_target));
      policy_z_nets.emplace_back(                                          //new modified
        new CFRNet(game, path, model_path, file_name + "_policy_z_1",
                   num_threads_, device, use_target));
    } else {
      value_nets.push_back(value_nets[0]);
      current_policy_nets.push_back(value_nets[0]);
      global_value_nets.push_back(global_value_nets[0]);
    }
  }
  if (use_policy_net) {
    policy_nets.emplace_back(new CFRNet(game, path, model_path,
                                        file_name + "_policy_0", num_threads_,
                                        device, use_target));
    if (seperate_policy_net) {
      policy_nets.emplace_back(new CFRNet(game, path, model_path,
                                          file_name + "_policy_1", num_threads_,
                                          device, use_target));
    } else {
      policy_nets.push_back(policy_nets[0]);
    }
  }
  value_table = std::shared_ptr<CFRTable>(new CFRTable(game));
  policy_table = std::shared_ptr<CFRTable>(new CFRTable(game));
  global_value_table = std::shared_ptr<CFRTable>(new CFRTable(game));
  current_policy_table = std::shared_ptr<CFRTable>(new CFRTable(game));
...
CFRNetModel::CFRNet::CFRNet(const Game& game, const std::string& path,
  ...
  if (file_name.find("global") != std::string::npos) {
