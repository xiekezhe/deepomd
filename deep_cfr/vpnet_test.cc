...
void TestModelCreation(const std::string& nn_model) {
  ...
  std::string obs_str = state->InformationStateString();
  CFRNetModel::InferenceInputs inputs = {obs_str, legal_actions, obs};
  ...
  train_inputs.emplace_back(CFRNetModel::TrainInputs{
      obs_str, legal_actions, legal_actions, obs,
      std::vector<double>(legal_actions.size(), 0.1), 1});
...
void TestModelLearnsSimple(const std::string& nn_model) {
  ...
  while (!state->IsTerminal()) {
    ...
    if (!state->IsChanceNode()) {
      ...
      train_inputs.emplace_back(CFRNetModel::TrainInputs{
          obs_str, legal_actions, legal_actions, obs, target, 1});
