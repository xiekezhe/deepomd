...
void UniversalPokerState::ObservationTensor(Player player,
  ...
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
  std::vector<int> tensor_shape = game_->ObservationTensorShape();
  int tensor_size = 0;
  for (auto &ts : tensor_shape) {
    tensor_size += ts;
  }
  ...
  int offset = 0;
  std::vector<uint8_t> hole_cards = acpc_state_.HoleCards(player);
  std::vector<uint8_t> opp_hole_cards =
      acpc_state_.HoleCards((player + 1) % num_players_);
  std::vector<uint8_t> board_cards = acpc_state_.BoardCards(num_board_cards_);
  for (uint32_t i = 0; i < hole_cards.size(); i++) {
    (*values)[i + offset] = hole_cards[i];
  }
  offset += acpc_game_->GetNbHoleCardsRequired();
  for (uint32_t i = 0; i < opp_hole_cards.size(); i++) {
    (*values)[i + offset] = opp_hole_cards[i];
  }
  offset += acpc_game_->GetNbHoleCardsRequired();
...
std::string UniversalPokerState::ObservationString(Player player) const {
  ...
  std::string result = absl::StrFormat(
      ...
      player != kChancePlayerId
          ? acpc_state_.HoleCardsToString((player + 1) % num_players_)
          : "",
      acpc_state_.BoardCardsToString(num_board_cards_), SequenceToString());
  return result;
}
...
int UniversalPokerGame::ObservationTensorSize() const {
  return acpc_game_.GetNbHoleCardsRequired() * acpc_game_.GetNbPlayers() +
         acpc_game_.GetTotalNbBoardCards() + MaxGameLength();
}

std::vector<int> UniversalPokerGame::ObservationTensorShape() const {
  std::vector<int> ret(acpc_game_.GetNbPlayers(),
                       acpc_game_.GetNbHoleCardsRequired());
  ret.push_back(acpc_game_.GetTotalNbBoardCards());
  ret.push_back(MaxGameLength());
  return ret;
}
