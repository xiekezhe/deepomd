...
void actor(const Game& game, const DeepCFRConfig& config,
  ...
  OnlineLearningOptimization::Mode mode =
      OnlineLearningOptimization::GetMode(config.cfr_mode);
  OnlineLearningOptimization::AverageType a_type =
      OnlineLearningOptimization::GetAverageType(config.average_type);
  OnlineLearningOptimization solver(
      game, value_0_eval, value_1_eval, policy_0_eval, policy_1_eval,
      config.use_regret_net, config.use_policy_net, config.use_tabular, &rng, 1,
      config.cfr_rm_scale, mode, a_type);
