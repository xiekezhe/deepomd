...
void actor(const Game& game, const DeepCFRConfig& config, int num,
  ...
  NFSPSolver solver(game, value_0_eval, value_1_eval, policy_0_eval,
                    policy_1_eval, config.use_regret_net, config.use_policy_net,
                    config.use_tabular, config.nfsp_anticipatory,
                    config.nfsp_eta, config.nfsp_epsilon, &rng);
      ...
      if (value_or_policy) {
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          if (node.next_action_index != -1) {
            ...
            target_value[0] =
                *std::max_element(ref_value.begin(), ref_value.end());
          } else {
            SPIEL_CHECK_TRUE(node.next_legal_actions.empty());
            SPIEL_CHECK_TRUE(target_outputs[i].value.empty());
          }
...
bool deep_cfr(DeepCFRConfig config, StopToken* stop) {
  ...
  if (config.graph_def.empty()) {
    config.graph_def = absl::StrJoin({config.game, std::string("nfsp")}, "_");
